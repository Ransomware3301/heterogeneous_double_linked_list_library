...
int main(int argc, char *argv[])
{
    ...
    printf("\n////// GENESIS_DL_LIST //////\n");
    print_dl_list_v2(head);

    data = create_data_block(
        safe_input(INT, STRING_BUFFER_SIZE, "Insert value to check (int): "),
        INT, 
        "%d"
    );
    ...
    printf("\n////// DELETE_INSTANCE //////\n");
    copy = copy_dl_list(head);
    copy = delete_instance(copy, data);
    print_dl_list_v2(copy);
    
    printf("\n////// FIND //////\n");
    printf("VALUE: %d | found ? => %d\n", *((int*)data->val), find(copy, data) != NULL);
    copy = delete_dl_list(copy);
    ...
    copy = copy_dl_list(head);
    copy = delete_all_instances(copy, data);
